...
class ChamHash_classic(ChamHash):
    ...
    def test_sendSK(self, user_url):
        data = {
            'ch_sk': Conversion.bytes2str(self.group.serialize(self.ch_sk['x']))
        }
        try:
            r = requests.post(user_url, data=json.dumps(data), headers={'content-type': 'application/json'})
...
class ABE(MAABE):
    def __init__(self):
        ...
        self.authorities = {}
        self.authorityAttributes = ["MINER", "USER"]
    ...
    def initialize_abe(self):
        ...
        self.new_transaction(
            ...
            data = {
                ...
                'ch_pk': {
                    'g': Conversion.bytes2str(self.group.serialize(self.ch_pk['g'])),
                    'h': Conversion.bytes2str(self.group.serialize(self.ch_pk['h']))
                }
            }
        )
        return GPP, GMK
...
class Blockchain(ChamHash_classic, ABE):
    def __init__(self):
        ...
        self.chain = []
        self.nodes = set()
        self.address = "ffffffffffffffffffffffffffffffff"

        # Initilize the chameleon hash keys
        (self.ch_pk, self.ch_sk) = self.paramgen()
        self.test_sendSK('http://localhost:5002/user_key_authKey')

        # Initilize the abe setup     
        self.ta_auth_info['GPP'], self.ta_auth_info['GMK'] = self.initialize_abe()
        self.setupAuthority()
        self.register_users("reimu_miner", 'http://localhost:5002/user_key_authKey')
        self.register_users("reimu_user", 'http://localhost:5001/user_key_authKey')
        # for attr in self.authorityAttributes[0:-1]:
        #     self.keygen_users(attr, "reimu", 'http://localhost:5001/user_key_authKey')
        self.keygen_users('MINER', "reimu_miner", 'http://localhost:5002/user_key_authKey')
        self.keygen_users('USER', "reimu_miner", 'http://localhost:5002/user_key_authKey')
        self.keygen_users('MINER', "reimu_user", 'http://localhost:5001/user_key_authKey') # for the revoking test
        self.keygen_users('USER', "reimu_user", 'http://localhost:5001/user_key_authKey')
...
def abe_encrypt():
    ...
    if len(blockchain.ta_auth_info['GPP']) or len(blockchain.ch_pk) or len(blockchain.authorities[blockchain.authorityID]):
        auth_info_tuple = blockchain.authorities[blockchain.authorityID]
        ...
        CT = blockchain.maabe.encrypt(blockchain.ta_auth_info['GPP'], values['attr_policy'], input_msg, auth_info_tuple)
...
def abe_decrypt():
    ...
    for tx in blockchain.chain[int(index)]['body']['transactions']:
        if not kv_ct in tx['data']:
            ...
            msg = blockchain.maabe.decrypt(blockchain.ta_auth_info['GPP'], CT, blockchain.users['reimu'])
            if not msg:
...
@app.route('/ukeygen', methods=['POST'])
def abe_ukeygen():
    values = request.get_json()

    # Check that the required fields are in the POST'ed data
    required = ['attribute', 'non_revoked_user']
    if not all(k in values for k in required):
        return 'Missing values', 400

    UK = blockchain.maabe.ukeygen(blockchain.ta_auth_info['GPP'], blockchain.authorities[blockchain.authorityID], values['attribute'], blockchain.users[values['non_revoked_user']]['user_public_info'])

    response = []
    # Send UKc to users
    data = {
        "revoked_attr": values['attribute'],
        "UKs": Conversion.bytes2str(blockchain.group.serialize(UK['UKs']))
    }
    try:
        r = requests.post("http://127.0.0.1:5002/user_key_authKey", data=json.dumps(data), headers={'content-type': 'application/json'})
    except ConnectionRefusedError:
            print("Connection refused")
        ...
        if r.status_code != 200:
            print("Fail to send user_key to the corresponding user")
            return "Fail to send user_key to the corresponding user refused", 400
        else:
            print(r.text)
    ...
    data = {
        "revoked_attr": values['attribute'],
        "UKc": [Conversion.bytes2str(blockchain.group.serialize(UK['UKc'][0])), Conversion.bytes2str(blockchain.group.serialize(UK['UKc'][1]))]
    }
    try:
        r = requests.post("http://127.0.0.1:5002/user_key_authKey", data=json.dumps(data), headers={'content-type': 'application/json'})
    except ConnectionRefusedError:
...
def forge_transaction():
    ...
    # Check that the required fields are in the POST'ed data
    required = ['block_index', 'cham_hash', 'kv_updated']
    if not all(k in values for k in required):
        return 'Missing values', 400
    ...
    for tx in original_block["body"]["transactions"]:
        if tx["cham_hash"] == '0':
            return 'This transaction is uneditable', 400
        if tx["cham_hash"] == values['cham_hash']:
            ...
            if not 'random' in original_data:
                ...
                random_new = blockchain.forge(blockchain.sk, \
